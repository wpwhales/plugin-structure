...
use WPWCore\Assets\Concerns\Conditional;
use WPWCore\Assets\Concerns\Enqueuable;
use WPWCore\Assets\Contracts\Bundle as BundleContract;
...
class Bundle implements BundleContract
{
    ...
    public function css(?callable $callable = null)
    {
        ...
        if (! $callable) {
            return \WPWCore\Collections\collect($styles);
        }

        \WPWCore\Collections\collect($styles)
    ...
    public function js(?callable $callable = null)
    {
        ...
        if (! $callable) {
            return \WPWCore\Collections\collect($scripts);
        }

        \WPWCore\Collections\collect($scripts)
            ->reject('runtime')
            ->each(function ($src, $handle) use ($callable) {
