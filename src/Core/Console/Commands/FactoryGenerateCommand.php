...
class FactoryGenerateCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($models as $model) {
            ...
            $filename = $this->laravel->basePath().'/database/factories/' . \WPWCore\Support\class_basename($model) . 'Factory.php';
    ...
    protected function generateFactory($model)
    {
        ...
        try {
            ...
            if (!$reflectionClass->isSubclassOf('WPWCore\Database\Eloquent\Model')) {
                return false;
            }
    ...
    protected function loadModels($models = [])
    {
        ...
        if (!empty($models)) {
            return array_map(function ($name) {
                // Check if the model name already contains a namespace
                if (strpos($name, '\\') !== false) {
                    return $name;
                }

                // Construct the full namespace for the model
                return $this->laravel->getNamespace() . "Models\\" . $name;
            }, $models);
        }
        ...
        // TODO : use this classmap later to find files if they are location somewhere else rather that the default directory
    ...
    protected function getPropertiesFromMethods($model)
    {
        ...
        foreach ($methods as $method) {
            if (!Str::startsWith($method, 'get') && !method_exists('WPWCore\Database\Eloquent\Model', $method)) {
                // Use reflection to inspect the code, based on WPWhales/Support/SerializableClosure.php
    ...
    public static function enumValues($table, $name)
    {
        ...
        /**
         * @var $x \WPWCore\Database\Query\Expression
         */


        $type = DB::select('SHOW COLUMNS FROM ' . $table . ' WHERE Field = "' . $name . '"')[0]->Type;
    ...
    protected function createFactory($class)
    {
        ...
        $content = view("factory")->with([
            'reflection' => $reflection,
            'properties' => $this->properties,
            'className'  => \WPWCore\Support\class_basename($class)
        ])->render();
