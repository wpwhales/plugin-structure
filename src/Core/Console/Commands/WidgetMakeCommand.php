...
class WidgetMakeCommand extends GeneratorCommand
{
     ...
     * @var string
     */
    protected $description = 'Create a new wordpress widget class ';
    ...
    public function handle()
    {
        ...
        if (!$this->files->exists($this->configPath("widgets.php"))) {
            $this->files->put($this->configPath("widgets.php"), '<?php return []; ?>');
        }
        ...
        $configPath = $this->laravel->configPath('widgets.php');
...
//
            $this->info("Widget name '{$name}' has been added to the config file.");
        } else {
            $this->info("Widget name '{$name}' already exists in the config file.");
        }
