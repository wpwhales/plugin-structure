...
class ScheduleListCommand extends Command
{
    ...
    {
        $events = \WPWCore\Collections\collect($schedule->events())
;
    ...
    private function getCronExpressionSpacing($events)
    {
        ...
        return \WPWCore\Collections\collect($rows[0] ?? [])
            ->keys()->map(fn ($key) => $rows->max($key))->all();
    }
    ...
    private function formatCronExpression($expression, $spacing)
    {
        ...
        return \WPWCore\Collections\collect($spacing)
     ...
     *
     * @param  \WPWCore\Console\Scheduling\CallbackEvent  $event
     * @return string
     */
    private function getClosureLocation(CallbackEvent $event)
    {
        $callback = \WPWCore\Support\tap((new ReflectionClass($event))->getProperty('callback'))
            ->setAccessible(true)
                        ->getValue($event);
