...
class Factory
{
     ...
     *
     * @var \WPWCore\Console\OutputStyle
     */
    protected $output;
     ...
     *
     * @param  \WPWCore\Console\OutputStyle  $output
     * @return void
     */
    ...
    {
        $component = '\WPWCore\Console\View\Components\\'.ucfirst($method);

        \WPWCore\Support\throw_unless(class_exists($component), new InvalidArgumentException(sprintf(
            'Console component [%s] not found.', $method
        )));

        return \WPWCore\Support\with(new $component($this->output))
            ->render(...$parameters);
    }
}
