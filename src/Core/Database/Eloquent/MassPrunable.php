...
trait MassPrunable
{
    ...
    {
        $query = \WPWCore\Support\tap($this->prunable(), function ($query) use ($chunkSize) {
...
;
        ...
        do {
            $total += $count = in_array(SoftDeletes::class, \WPWCore\Support\class_uses_recursive(get_class($this))
)
                        ? $query->forceDelete()
                        : $query->delete();
