...
class LogManager implements LoggerInterface
{
    ...
    {
        try {
            return $this->channels[$name] ?? \WPWCore\Support\with($this->resolve($name, $config), function ($logger) use ($name) {
                return $this->channels[$name] = $this->tap($name, new Logger($logger, $this->app['events']))->withContext($this->sharedContext);
            });
        } catch (Throwable $e) {
            return \WPWCore\tap($this->createEmergencyLogger(), function ($logger) use ($e) {
     ...
     *
     * @param  string  $name
     * @param  \WPWCore\Log\Logger  $logger
     * @return \WPWCore\Log\Logger
     */
    ...
    protected function createEmergencyLogger()
    {
        ...
        $handler = new StreamHandler(
            $config['path'] ?? $this->app->storagePath().'/logs/wpwcore.log',
    ...
    protected function createStackDriver(array $config)
    {
        ...
        $handlers = \WPWCore\Collections\collect($config['channels'])->flatMap(function ($channel) {
            return $channel instanceof LoggerInterface
                ? $channel->getHandlers()
                : $this->channel($channel)->getHandlers();
        })->all();

        $processors = \WPWCore\Collections\collect($config['channels'])->flatMap(function ($channel) {
            return $channel instanceof LoggerInterface
    ...
    protected function createMonologDriver(array $config)
    {
        ...
        \WPWCore\Collections\collect($config['processors'] ?? [])->each(function ($processor) {
        ...
        $processors = \WPWCore\Collections\collect($config['processors'] ?? [])
