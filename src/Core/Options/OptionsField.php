...
class OptionsField implements OptionsFieldContract
{
    ...
    public function registerOptionField(OptionsFieldContract $field, $page, $section, $optionGroup, $optionName, array $args, $renderFunction)
    {
    ...
    public function registerOptionFieldAdminInit(){

            $this->registerOptionField(...$this->fieldData);
    ...
    final public function register($optionsPage, $optionsSection, $optionGroup, $optionName, $hook = true)
    {
        ...
        if ($this->renderFunction) {
            ...
            $optionsFieldGeneratorClassName = '\\WPWCore\\Options\\OptionsFieldGenerator\\' . ucfirst(strtolower($this->type())) . 'FieldGenerator';

            if (!class_exists($optionsFieldGeneratorClassName)) {
                throw new InvalidArgumentException("Invalid field type `{$field->type()}`.");
            }

            /** @var \WPWCore\Options\Contracts\OptionsFieldGenerator $optionsFieldGenerator */
        ...
        $this->fieldData = [$field, $page, $section, $optionGroup, $optionName, $args, $renderFunction];

        if ($hook) {
            add_action('admin_init', [$this,"registerOptionFieldAdminInit"]);
        } else {
            $this->registerOptionField(...$this->fieldData);
        }
