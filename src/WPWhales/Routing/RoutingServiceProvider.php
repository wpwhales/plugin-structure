...
use WPWhales\Contracts\View\Factory as ViewFactoryContract;
        ...
        $this->app->bind(ServerRequestInterface::class, function ($app) {
            if (class_exists(Psr17Factory::class) && class_exists(PsrHttpFactory::class)) {
                ...
                return with((new PsrHttpFactory($psr17Factory, $psr17Factory, $psr17Factory, $psr17Factory))
                    ->createRequest($WPWhalesRequest = $app->make('request')), function (ServerRequestInterface $request) use ($WPWhalesRequest) {
                        if ($WPWhalesRequest->getContentTypeFormat() !== 'json' && $WPWhalesRequest->request->count() === 0) {
                            return $request;
                        }

                        return $request->withParsedBody(
                            array_merge($request->getParsedBody() ?? [], $WPWhalesRequest->getPayload()->all())
                        );
                    });
            }
