...
class Benchmark
{
    ...
    {
        return \WPWCore\Collections\collect(Arr::wrap($benchmarkables))
            ->map(function ($callback) use ($iterations) {
                return \WPWCore\Collections\collect(range(1, $iterations))
                    ->map(function () use ($callback) {
                        gc_collect_cycles();
    ...
    {
        $result = \WPWCore\Collections\collect(static::measure(Arr::wrap($benchmarkables), $iterations))
