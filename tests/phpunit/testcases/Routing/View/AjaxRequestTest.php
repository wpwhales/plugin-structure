...
class AjaxRequestTest extends \WP_Ajax_UnitTestCase
{
    ...
    public function test_view_in_response()
    {
        ...
        $this->app->router->get("/view_in_response", [
            "uses" => TestController::class . "@view_in_response"
        ]);


        $response = $this->adminAjaxCall("GET", "/view_in_response");
        $response->assertStatus(200);
        $response->assertExactJson(["html" => "<h1>Hello World</h1>"]);
    ...
    public function test_view_with_method()
    {
        ...
        $this->app->router->get("/view_with_method", [
            "uses" => TestController::class . "@view_with_method"
        ]);


        $response = $this->adminAjaxCall("GET", "/view_with_method");
